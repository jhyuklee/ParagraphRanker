...
class ParagraphRanker(object):
    ...
    def update(self, ex):
        ...
        if self.use_cuda:
            inputs = [e if e is None else Variable(e.cuda(async=True))
                      for e in ex[:5]]
            targets = Variable(ex[5].cuda(async=True))
        else:
            inputs = [e if e is None else Variable(e) for e in ex[:5]]
            targets = Variable(ex[5])

        # Run forward
        doc_scores, _, _ = self.network(*inputs)

        # Compute loss and accuracies
        loss = F.binary_cross_entropy(doc_scores, targets)
    ...
    def encode(self, ex):
        ...
        # Run forward
        _, doc_encoded, q_encoded = self.network(*inputs)

        return doc_encoded, q_encoded
